...
class InputController {
	...
	constructor() {
		...
		sceneDiv?.addEventListener('pointerdown', (e: PointerEvent) => this._onPointerDown(e));
		sceneDiv?.addEventListener('pointerup', (e: PointerEvent) => this._onPointerUp(e));
		sceneDiv?.addEventListener('pointermove', (e: PointerEvent) => this._onPointerMove(e), false);

		const controlsPanel = document.createElement('div');
		controlsPanel.id = 'controls-panel';
		document.body.appendChild(controlsPanel);
		controlsPanel.addEventListener('pointerdown', (e: PointerEvent) => this._onPointerDown(e));
		controlsPanel.addEventListener('pointerup', (e: PointerEvent) => this._onPointerUp(e));
		...
		walkControl.addEventListener('pointerdown', (e: PointerEvent) => this._onPointerDown(e));
		walkControl.addEventListener('pointermove', (e: PointerEvent) => this._onPointerMove(e));
	...
	private _onPointerDown(e: PointerEvent) {
		...
		if (eTarget.id === 'walk-control' || eTarget.id === 'walk-control-ref') isJostick = true;
	...
	private _onPointerUp(e: PointerEvent) {
		const activePointer = this._pointers.filter((pointer) => pointer.id === e.pointerId)[0];
		...
		this._pointers = this._pointers.filter((pointer) => pointer.id != e.pointerId);
...
class Viewer {
	...
	private _updateRotation() {
		...
		this._theta = clamp(this._theta + -yh * THETA_SPEED, -Math.PI / 10, Math.PI / 30);
	...
	private _updateRayIntersections() {
		...
		const intersections = raycaster.intersectObjects(GalleryScene.instance.children, true);
	...
	public moveToObject3D(target: Object3D) {
		...
		const SPEED =
			distance < FAST_TRANSLATION_RANGE ? TRANSLATION_SPEED * 10 : FAST_TRANSLATION_SPEED * 10;
		...
		const step = () => {
			...
			if (delta < 1) {
				...
				const testigPosition = this._position.clone();
				const testingSphere = new Sphere(testigPosition, COLLISION_DISTANCE/2);
				const colliders = GalleryScene.instance.entities.filter((e) =>
					e.collider?.intersectsSphere(testingSphere)
				);
