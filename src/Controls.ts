...
export default class Controls {
	...
	private _isContentInfoOpen = false;
	...
	constructor() {
		...
		controlZoom.addEventListener(
			'click',
			() => {
				this._zoomActiveContent(controlZoom);
			},
			false
		);
		...
		controlButtomLeft.className = 'icon-left-arrow';
		...
		controlButtomRight.className = 'icon-right-arrow';
	...
	public castShadowWallLights(wallActiveIndex = 0) {
		...
		this._wallsObj3D.forEach((item) => {
			const spotLightsBox = item.children.filter((item) => item.userData.type === 'spotLight');
			spotLightsBox.forEach((item) => {
				actualValue = wallIndex === wallActiveIndex ? true : false;
				item.children.filter((item) => item.type === 'SpotLight')[0].castShadow = actualValue;
			});
			wallIndex++;
		});
	}
	...
	public moveToNextContent(directionSign = 1) {
		...
		if (!this._activeContentId) {
			const wall = GalleryScene.instance.galleryData?.room?.walls[this._activeWallIndex];
			if (!wall) return;
		...
		const restOfContentsInActiveWall = this._wallsObj3D[this._activeWallIndex].children.filter(
			(item) => item.userData.type == 'content' && item.userData.id !== this._activeContentId
		);
		//case 1: there are more than one content in the active wall.
		if (restOfContentsInActiveWall.length > 0) {
			const activeContent = this._wallsObj3D[this._activeWallIndex].children.filter(
			...
			restOfContentsInActiveWall.forEach((item) => {
				if (
					Math.sign(item.position.x - activeContent.position.x) == directionSign &&
	...
	private _moveToNextWall(directionSign = 1) {
		...
		const nextWall = GalleryScene.instance.galleryData?.room?.walls[nextWallIndex];
		if (!nextWall) return;
	...
	public showContentInfo(contentId: string) {
		...
		if (contentInfo && !content.disabled) {
			...
			this._isContentInfoOpen = true;
			let innerHTML = `<strong>${contentInfo.name}</strong>`;
			innerHTML += contentInfo.description ? `<p>${contentInfo.description}</p>` : '</br></br>';
			const { showDimensions, height, width, depth } = { ...contentInfo.realDimensions };
			if (showDimensions) {
				if (height && width && depth) {
					innerHTML += `<small>${height} x ${width} x ${depth} cm - 
					${(height * 0.39).toFixed(2)} x ${(width * 0.39).toFixed(2)} x ${(depth * 0.39).toFixed(2)} in</small>`;
				}
				if (height && width) {
					innerHTML += `<small>${height} x ${width} cm - ${(height * 0.39).toFixed(2)} x ${(width * 0.39).toFixed(2)} in</small>`;
				}
			}
			if (contentInfo.price && contentInfo.price != '0') innerHTML += `<p>${contentInfo.price}</p>`;
			...
			setTimeout(() => {
				...
				this._isContentInfoOpen = false;
			}, 5000);
		}
	}

	private _zoomActiveContent(controlZoom: HTMLDivElement) {
	...
	public initRoomAudio() {
		...
		const generalAudioId = GalleryScene.instance.galleryData?.room?.generalAudioId;
		console.log(generalAudioId);
		if (generalAudioId && this._firstLoad) {
